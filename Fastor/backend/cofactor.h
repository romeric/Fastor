...
FASTOR_HINT_INLINE void _cofactor(const T * FASTOR_RESTRICT a, T * FASTOR_RESTRICT out) {
    if (M==2 && N==2) {
        ...
        out[1] = -(a[3]*a[8] - a[6]*a[5]);
        out[2] =  (a[3]*a[7] - a[6]*a[4]);
        out[3] = -(a[1]*a[8] - a[7]*a[2]);
        out[4] =  (a[0]*a[8] - a[6]*a[2]);
        out[5] = -(a[0]*a[7] - a[6]*a[1]);
        out[6] =  (a[1]*a[5] - a[4]*a[2]);
        out[7] = -(a[0]*a[5] - a[3]*a[2]);
...
FASTOR_HINT_INLINE void _cofactor<float,2,2>(const float * FASTOR_RESTRICT a, float * FASTOR_RESTRICT out) {
    ...
    _mm_store_ps(out,_mm_shuffle_ps(a1,a1,_MM_SHUFFLE(0,3,2,1)));
}
...
FASTOR_HINT_INLINE void _cofactor<float,3,3>(const float * FASTOR_RESTRICT a, float * FASTOR_RESTRICT out) {
    ...
    _mm_store_ss(out+1,c_01);
    _mm_store_ss(out+2,c_02);
    _mm_store_ss(out+3,c_10);
    _mm_store_ss(out+4,c_11);
    _mm_store_ss(out+5,c_12);
    _mm_store_ss(out+6,c_20);
    _mm_store_ss(out+7,c_21);
...
FASTOR_HINT_INLINE void _cofactor<double,2,2>(const double * FASTOR_RESTRICT a, double * FASTOR_RESTRICT out) {
    ...
    _mm_store_sd(out+1,_mm_neg_pd(_mm_load_sd(a+2)));
    _mm_store_sd(out+2,_mm_neg_pd(_mm_load_sd(a+1)));
}
...
FASTOR_HINT_INLINE void _cofactor<double,3,3>(const double * FASTOR_RESTRICT a, double * FASTOR_RESTRICT out) {
    ...
    _mm_store_sd(out+1,c_01);
    _mm_store_sd(out+2,c_02);
    _mm_store_sd(out+3,c_10);
    _mm_store_sd(out+4,c_11);
    _mm_store_sd(out+5,c_12);
    _mm_store_sd(out+6,c_20);
