...
void run() {
...

    {
        ...
        A(all,0) = v;
        FASTOR_EXIT_ASSERT(abs(A.sum() - 5635) < Tol);
        A(all,0) += v;
        FASTOR_EXIT_ASSERT(abs(A.sum() - 6680) < Tol);
        A(-1,all) -= v;
        FASTOR_EXIT_ASSERT(abs(A.sum() - 5635) < Tol);
        A(seq(0,-1,2),2) *= v(seq(0,5));
        FASTOR_EXIT_ASSERT(abs(A.sum() - 27550) < Tol);
        A(4,seq(0,last,1)) /= 100*v;
        FASTOR_EXIT_ASSERT(abs(A.sum() - 22585.484375) < HugeTol);
        A(last,last) = v(last);
        FASTOR_EXIT_ASSERT(abs(A.sum() - 22703.484375) < HugeTol);
        ...
        A.iota(1);
        v = A(all,0);
        FASTOR_EXIT_ASSERT(abs(v.sum() - 460) < Tol);
        v = A(1,all);
        FASTOR_EXIT_ASSERT(abs(v.sum() - 155) < Tol);
        v += A(2,all);
        FASTOR_EXIT_ASSERT(abs(v.sum() - 410) < Tol);
        v -= A(all,3);
        FASTOR_EXIT_ASSERT(abs(v.sum() + 80) < Tol);
        v *= A(all,0);
        FASTOR_EXIT_ASSERT(abs(v.sum() + 10280) < BigTol);
        v /= -A(all,1);
        FASTOR_EXIT_ASSERT(abs(v.sum() - 94.38235) < HugeTol);
...

    {
        ...
        B(all,all,1) = A;
        FASTOR_EXIT_ASSERT(abs(B.sum() - 45050) < Tol);
        B(all,all,0) += A;
        FASTOR_EXIT_ASSERT(abs(B.sum() - 60000) < Tol);
        B(all,0,all) -= A(all,seq(0,3));
        FASTOR_EXIT_ASSERT(abs(B.sum() - 55620) < Tol);
        B(all,all,2) *= A(all,all) + 2;
        FASTOR_EXIT_ASSERT(abs(B.sum() - 2362800) < Tol);
        B(all,1,1) = 50;
        FASTOR_EXIT_ASSERT(abs(B.sum() - 2361840) < Tol);
        B(all,1,1) /= -200*A(1,all);
        FASTOR_EXIT_ASSERT(abs(B.sum() - 2361339.97815) < 0.5);

        // Assigning high order tensor views to matrices 
        B.iota(1);
        A = B(all,all,1);
        FASTOR_EXIT_ASSERT(abs(A.sum() - 15050) < Tol);
        A += C(all,0,all,1);
        FASTOR_EXIT_ASSERT(abs(A.sum() - 184000) < Tol);
        A -= C(all,0,all,1);
        FASTOR_EXIT_ASSERT(abs(A.sum() - 15050) < Tol);
        A *= C(all,0,all,1) + B(all,all,0);
        FASTOR_EXIT_ASSERT(abs(A.sum() - 32146800) < BigTol);
        A /= C(all,0,all,1) - B(all,all,0);
        FASTOR_EXIT_ASSERT(abs(A.sum() - 18377.20703) < 0.5);
        ...
        FASTOR_EXIT_ASSERT(abs(C.sum() - 1094350) < HugeTol);
        C(seq(0,4,2),all,0,0) += A(seq(1,3),seq(first,5,1));
        FASTOR_EXIT_ASSERT(abs(C.sum() - 1094520) < HugeTol);
        C(seq(0,4,2),all,0,0) -= A(seq(1,3),seq(first,5,1));
        FASTOR_EXIT_ASSERT(abs(C.sum() - 1094350) < HugeTol);
        ...
        FASTOR_EXIT_ASSERT(abs(A.sum() + 1305.46) < 2.);
    }
