...
void run() {
...

    {
        ...
        // Scalar indexing
        assert(abs(r1(0,2,1,3,4) - 119) < Tol);

        // Views
        // Assignment to views
        r1(all,0,all,all,3) = -1234;
        assert(abs(norm(r1) - 8491.192377) < BigTol);
        ...
        assert(abs(norm(r1) - 9444.503) < 1e-3);
        r1(seq(0,3,2),all,1,seq(1,4,3),4) -= 1234;
        assert(abs(norm(r1) - 8491.192377) < BigTol);
        r1(seq(0,3,2),0,1,seq(1,4,3),4) *= 12;
        assert(abs(norm(r1) - 9467.742) < 1e-3);
        r1(seq(0,3,2),0,1,seq(1,4,3),4) /= 12;
        assert(abs(norm(r1) - 8491.192377) < BigTol);
        ...
        assert(abs(norm(r2) - norm(r3)) < Tol);
        r3 = 2*r2(all,seq(0,-1),seq(0,5))/12 + 5;
        assert(abs(norm(r3) - 170.560351) < 1e-3);
        r3 += -3*r3(all,all,all) - 15;
        assert(abs(norm(r3) - 441.462) < 1e-3);
        r3 -= -3*r3(all,all,all) - 15;
        assert(abs(norm(r3) - 1665.45439) < 1e-3);
        r3 *= r2/5 - 6*r3(all,all,all) + r2(all,all,all);
        assert(abs(norm(r3) -  2753466.75326) < 0.5);
        r3 /= r2/5 - 6*r3(all,all,all) / r2(all,all,all);
        assert(abs(norm(r3) -  137.001) < 1e-2);

        // Assigning to a view from numbers/tensors/views
        r3(seq(0,-1,1),all,seq(0,5)) = 2;
        assert(abs(norm(r3) - 13.416407) < 1e-3);
        r3(all,all,all) = r3(all,all,all);
        assert(abs(norm(r3) - 13.416407) < 1e-3);
        r3(0,seq(0,2),seq(1,-1,2)) += 5;
        assert(abs(norm(r3) - 18.9736659) < 1e-3);
        r3(0,seq(0,2),seq(1,-1,2)) -= 5;
        assert(abs(norm(r3) - 13.416407) < 1e-3);
        r3(0,seq(0,2),seq(1,-1,2)) *= 5;
        assert(abs(norm(r3) - 23.748684) < 1e-3);
        r3(0,seq(0,2),seq(1,-1,2)) /= 5;
        assert(abs(norm(r3) - 13.416407) < 1e-3);
        ...
        assert(abs(norm(r2) - norm(r3)) < Tol);
        r3(all,all,all) += r2;
        assert(abs(norm(r3) - 1646.049816) < 1e-3);
        r3(all,all,all) -= r2;
        assert(abs(norm(r2) - norm(r3)) < BigTol);
        r3(all,all,all) *= r2;
        assert(abs(norm(r3) - 103194.60105) < 0.5);
        r3(all,all,all) /= r2;
        assert(abs(norm(r2) - norm(r3)) < BigTol);
        ...
        assert(abs(norm(r3) - 1218.00862) < 1e-3);
        ...
        for (int i=0; i<2; ++i) {
            for (int j=0; j<3; ++j) {
                for (int k=0; k<4; ++k) {
                    for (int l=0; l<1; ++l) {
                        assert(abs(r4(i,j,k,l) - counter) < Tol);
                        assert(abs(r4(last-i,last-j,last-k,last-l) - (23-counter) ) < Tol);
                        counter++;
                    }
                }
            }
